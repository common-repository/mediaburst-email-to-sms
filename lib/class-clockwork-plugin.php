...
abstract class Clockwork_Plugin {
  ...
  public function clockwork_test_message( $to ) {
    ...
    if( !$clockwork->is_valid_msisdn( sanitize_text_field($_GET['to']) ) ) {
      $log[] = esc_html($_GET['to']) . ' appears an invalid number to send to, this message may not send';
    }

    $log[] = 'Attempting test send with API key ' . $options['api_key'] . ' to ' . esc_html($_GET['to']);

    try {
      $message_data = array( array( 'from' => $options['from'], 'to' => sanitize_text_field($_GET['to']), 'message' => $message ) );
